						...
						if(this.options.parseDirection ==	'x'){
								this.table.find('tr:gt(0)').filter(rowFilter).each(function(i, tr){
										...
										$(tr).find('td').filter(colFilter).each(function(){
											...
											dataGroups[i].points.push( values.length > 0 ? values[0] : 0 );
											dataGroups[i].points_weight.push(	values.length	>	1	?	values[1]	:	1	);
										});
								});
						}
						else {
							...
							for(var	i=0; i<cols; i++){
								...
								self.find('tr:gt(0)').filter(o.rowFilter).each(function(){
									var	values = $(this).find('td').filter(o.colFilter).eq(i).text().split(	";"	);							
									dataGroups[i].points.push( values.length > 0 ? values[0]*1 : 0 );
									dataGroups[i].points_weight.push(	values.length	>	1	?	values[1]	:	1	);
								});
							};
						}
